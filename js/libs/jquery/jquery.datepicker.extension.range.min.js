    ...
    ($.datepicker._setDate = function (t, e, i) {
        ...
        switch (a) {
            ...
            case "multiple":
                ...
                // console.log("timenodes", useTimeLogContext.current.timenodes)
                ...
                useTimeLogContext.current.timenodes.forEach(el => {
                    ...
                    let contextMatch = el.smena == useTimeLogContext.current.smena && el.workType == useTimeLogContext.current.workType && useTimeLogContext.current.object == el.object
                    totalMatchFound = contextMatch && el.date == useTimeLogContext.current.date ? true : totalMatchFound
                    if (contextMatch) {
                        if (el.hours != null && el.date != useTimeLogContext.current.date) {
                            // console.log(useTimeLogContext.current)
                            console.log("очередная pre-defined date", el)
                ...
                s.setClassActive(t);
                if (t.settings.init == true) { s.dates.pop(n._getDate(t)), s.datesText.pop(n._formatDate(t)), t.settings.init = false; };
                // console.log(1234)
                // if
        ...
        (this.fillDayMultiple = function (t, e) {
            var i = e[1],
                ...
                useTimeLogContext.current.timenodes.forEach(e => {
                    let date = e.date.split(".")
                    date = new Date(date[2], date[1]-1, date[0])
                    if ( date.getTime() == t.getTime() && e.hours != null) { // Если даты совпадают и есть инфа по часам
                        // console.log("utl", useTimeLogContext)
                        // console.log("t", t)
                        deselect = 0
                        // console.log("mdeee//// e fdm", e)
                    }
                })

                // console.log(useTimeLogContext.current.timenodes)

            return (
                $.map(this.dates, function (e) {
                    e.getTime() == t.getTime() && (a = !0); // Если даты совпадают
                }),
                !deselect && a && (i += " selected selected-start selected-end"), // Здесь цепляются теги в случае совпадения дат - только при клике.
                (e[1] = i.trim()),
                e
            );
        }),
