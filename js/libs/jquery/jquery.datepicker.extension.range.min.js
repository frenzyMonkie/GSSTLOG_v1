...
($.datepicker._get_original = $.datepicker._get),
    ($.datepicker._get = function (t, e) {
        ...
        // console.log(this._get( inst, "useTimeLogContext" ))
        // console.log("T",  t.settings.useTimeLogContext)
        // console.log(e)
        var useTimeLogContext = t.settings.useTimeLogContext
        // console.log("t.useTimeLogContext", t)
        switch (a) {
            case "period":
            case "multiple":
                ...
                switch ((n || ((n = new _datepickerExtension(useTimeLogContext)), $(this.dpDiv).data("datepickerExtensionRange", n)), (n.range = a), (n.range_multiple_max = t.settings.range_multiple_max || 0), e)) {
                    case "onSelect":
                        var r = i;
                        r || (r = function () {}),
                            (i = function (t, e) {
                                n.onSelect(t, e),
                                    ...
                                    setTimeout(function () {
                                        ...
                                        // console.log("datepicker", useTimeLogContext)
                                        useTimeLogContext.datepicker = s
                                        useTimeLogContext.inst = e
    ...
    ($.datepicker._setDate = function (t, e, i) {
        ...
        var useTimeLogContext = this._get( t, "useTimeLogContext" );
        ...
        switch (a) {
            ...
            case "multiple":
                ...
                console.log("_setDate timenodes 1", useTimeLogContext.current.timenodes)

                // ПРОИСХОДИТ ТОЛЬКО 1 РАЗ ПРИ СТАРТЕ КАЛЕНДАРЯ, ВИДИМО.
                // console.log("123 useTimeLogContext",useTimeLogContext)
                let totalMatchFound = false
                useTimeLogContext.current.timenodes.forEach(item => {
                    // console.log("123", useTimeLogContext.current.timenodes)
                    ...
                    let contextMatch = item.smena == useTimeLogContext.current.smena && item.workType == useTimeLogContext.current.workType && useTimeLogContext.current.object == item.object
                    totalMatchFound = contextMatch && item.date == useTimeLogContext.current.date ? true : totalMatchFound
                    if (contextMatch) {
                        if (item.hours != null && item.date != useTimeLogContext.current.date) {
                ...
                //         date: useTimeLogContext.current.date,
                //         hours: useTimeLogContext.current.hours,
                //     }
                ...
                // console.log(1234)
                console.log("_setDate timenodes 2", useTimeLogContext.current.timenodes)

        }
    });
var _datepickerExtension = function (useTimeLogContext) {
    ...
    (this.range = !1),
        ...
        (this.onSelectMultiple = function (t, e) {
            ...
            var useTimeLogContext = e.settings.useTimeLogContext
            ...
            useTimeLogContext.current.timenodes.forEach(e => {
                ...
                if ( date == t && e.hours != null) { // Если даты совпадают и есть инфа по часам
                    // console.log("utl", useTimeLogContext)
                    // console.log("t", t)
                    deselect = 0
            ...
            // useTimeLogContext.setCurrentTimenodesByDate(useTimeLogContext, t)
        ...
        (this.fillDayMultiple = function (t, e) {
            var i = e[1],
                ...
                useTimeLogContext.current.timenodes.forEach(e => {
                    let date = e.date.split(".")
                    date = new Date(date[2], date[1]-1, date[0])
                    let currentContext = useTimeLogContext.current
                    let contextMatch = e.object == currentContext.object && e.smena == currentContext.smena && e.workType == currentContext.workType
                    let totalMatch = contextMatch && e.date == useTimeLogContext.current.date
                    let dateMatch = e.date == useTimeLogContext.current.date
                    if ( date.getTime() == t.getTime() && contextMatch) { // Если даты совпадают и есть инфа по часам


                        // console.log("t", t)
                        let action = useTimeLogContext.current.hoursAction
                        let removeCase = action == "remove" && contextMatch && useTimeLogContext.current.hours == null
                        let updateCase = action == "update" && contextMatch && useTimeLogContext.current.hours != null
